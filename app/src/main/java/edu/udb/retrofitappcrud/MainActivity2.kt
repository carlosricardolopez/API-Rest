...
class MainActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val retrofit = Retrofit.Builder()
            ...
            //.baseUrl("http://192.168.0.2/kotlin/")
            .baseUrl("https://kotlindsm1.000webhostapp.com/profesores/")
        ...
        fab_agregar.setOnClickListener(View.OnClickListener {
            val i = Intent(getBaseContext(), CrearProfesor::class.java)
    ...
    private fun eliminarAlumno(alumno: Alumno, api: ProfesorApi) {
        ...
        llamada.enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@MainActivity2, "Alumno eliminado", Toast.LENGTH_SHORT).show()
                    cargarDatos(api)
                } else {
                    val error = response.errorBody()?.string()
                    Log.e("API", "Error al eliminar alumno : $error")
                    Toast.makeText(this@MainActivity2, "Error al eliminar alumno 1", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<Void>, t: Throwable) {
                Log.e("API", "Error al eliminar alumno : $t")
                Toast.makeText(this@MainActivity2, "Error al eliminar alumno 2", Toast.LENGTH_SHORT).show()
            }
        })
    }
}